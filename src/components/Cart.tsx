...
export function Cart() {
  ...
  const handleWhatsAppCheckout = (phone: string) => {
    ...
    const itemList = items.map((item, index) => {
      ...
      const priceText = `₹${formatNumberIntl(item.price || 0, language)}`;
      const qtyText = formatNumberIntl(item.quantity, language);
      return `${formattedIndex}. ${item.name}%0A   ${priceText} x  ${qtyText}`;
    }).join('%0A%0A');
                  ...
                  <div key={item.id} className={cn("p-4 flex gap-4", index !== items.length - 1 && "border-b border-gray-100")}>
                    ...
                    <div className="flex flex-col flex-1">
                      <div className="flex justify-between items-start">
                        ...
                        {item.price && <span className="text-sm font-semibold text-gray-900 shrink-0">₹{formatNumberIntl((item.price * item.quantity).toFixed(2), language)}</span>}
                      </div>
                      <div className="mt-4 flex items-center gap-4">
                        <div className="flex items-center border border-gray-200 rounded-sm bg-white shrink-0">
                          ...
                          <span className="text-sm font-medium w-10 text-center">{formatNumberIntl(item.quantity, language)}</span>
                          <button 
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
          ...
          {items.length > 0 && (
            <div className="absolute bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 drop-shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-10 flex justify-between items-center h-20">
              <div className="flex flex-col">
                <span className="text-sm text-gray-500 font-medium">{formatNumberIntl(totalItems(), language)} {getTranslation(language, 'items')}</span>
                <span className="text-lg font-bold text-black">
                  ₹{formatNumberIntl(items.reduce((total, item) => total + (item.price || 0) * item.quantity, 0).toFixed(2), language)}
                </span>
              </div>
                ...
                <AnimatePresence>
                  {showContacts && (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, y: 10 }}
                      className="absolute bottom-full right-0 mb-3 w-48 bg-white border border-gray-200 rounded-md shadow-lg overflow-hidden flex flex-col z-50 origin-bottom"
                    >
                      <div className="px-3 py-2 bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">Select Contact</div>
                      <button onClick={() => handleWhatsAppCheckout('919028646863')} className="px-4 py-3 text-sm text-left hover:bg-gray-50 font-medium text-gray-800 border-b border-gray-100 transition-colors">Via Umesh</button>
                      <button onClick={() => handleWhatsAppCheckout('918983424489')} className="px-4 py-3 text-sm text-left hover:bg-gray-50 font-medium text-gray-800 border-b border-gray-100 transition-colors">Via Ram</button>
                      <button onClick={() => handleWhatsAppCheckout('919172086464')} className="px-4 py-3 text-sm text-left hover:bg-gray-50 font-medium text-gray-800 border-b border-gray-100 transition-colors">Via Shyam</button>
                      <button onClick={() => handleWhatsAppCheckout('919503713123')} className="px-4 py-3 text-sm text-left hover:bg-gray-50 font-medium text-gray-800 transition-colors">Via Dipu</button>
                    </motion.div>
                  )}
                </AnimatePresence>
                <button 
                  onClick={() => setShowContacts(!showContacts)}
                  className="w-full py-3 text-white rounded-sm font-bold flex items-center justify-center space-x-2 bg-[#25D366] hover:bg-[#20bd5c] transition-colors shadow-sm"
                >
                  ...
                  <span className="truncate">{getTranslation(language, 'orderViaWhatsapp')}</span>
                </button>
              </div>
            </div>
          )}
